...
=end
  ...
  def return_otp
    ...
    @otps = Log.where("(transaction_type = 'Credit de compte' OR transaction_type = 'Debit du compte') AND account_number = '#{account_number}'").order("created_at DESC").limit(5) rescue nil
  end
